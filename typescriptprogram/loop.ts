...
let num3: number = 5; // Change this value to test
...
while (numberToReverse > 0) {
    ...
    numberToReverse = Math.floor(numberToReverse / 10); // Remove last digit Divides the number by 10 and removes the decimal part (Math.floor ensures it’s an integer).
}
...
for (let i= 1; i <= 30; i++) {
    if (i%5===0) {
       ...
       continue;
         ...
         // Exit loop once the first even number is found
    }
    console.log(i);
